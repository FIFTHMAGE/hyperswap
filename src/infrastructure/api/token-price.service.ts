...
export async function getTokenPrices(
  ...
  return withRetry(async () => {
    ...
    const response = await get<PriceData>(coingeckoClient, '/simple/token_price/' + platform, {
      params: {
...
export async function getNativeTokenPrice(chainId: ChainId): Promise<number> {
  return withRetry(async () => {
    ...
    const response = await get<any>(coingeckoClient, '/simple/price', {
      params: {
        ids: coinId,
        vs_currencies: 'usd',
      },
 ...
 */
export async function getTokenMarketData(chainId: ChainId, tokenAddress: string): Promise<any> {
  return withRetry(async () => {
    const platform = getPlatformId(chainId);

    const response = await get<any>(coingeckoClient, `/coins/${platform}/contract/${tokenAddress}`);
