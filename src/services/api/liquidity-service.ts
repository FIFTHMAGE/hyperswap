...
export class LiquidityService {
   ...
   */
  async getPoolAnalytics(poolAddress: string, timeframe: '24h' | '7d' | '30d' = '7d'): Promise<PoolAnalytics | null> {
    try {
  ...
  async getUserPositions(userAddress: string): Promise<PoolPosition[]> {
    try {
      const response = await fetch(
        `${this.apiBaseUrl}/positions/${userAddress}`
      );
  ...
  async searchPoolsByTokens(token0: string, token1: string): Promise<LiquidityPool[]> {
    try {
      const response = await fetch(
        `${this.apiBaseUrl}/search?token0=${token0}&token1=${token1}`
      );
   ...
   */
  async getTopPools(metric: 'tvl' | 'volume' | 'apy' = 'tvl', limit: number = 10): Promise<LiquidityPool[]> {
    try {
      const response = await fetch(
        `${this.apiBaseUrl}/top?metric=${metric}&limit=${limit}`
      );
