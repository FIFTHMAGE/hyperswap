...
const TokenBalances: React.FC = () => {
  ...
  const { balances, loading: _loading } = useTokenBalances(wallet?.address || '');
  ...
  return (
    <Card padding="md">
      <div className="flex items-center justify-between mb-4">
        ...
        <SearchInput placeholder="Search tokens..." onSearch={setSearchQuery} className="w-64" />
      </div>
          ...
          >
            ...
            <div className="text-right">
              ...
              {token.priceChange24h !== undefined && (
                <p
                  className={`text-sm ${token.priceChange24h >= 0 ? 'text-green-600' : 'text-red-600'}`}
                >
